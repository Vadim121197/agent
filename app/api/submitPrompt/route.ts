...
export async function POST(request: Request) {
	try {
		...
		const messages: Message[] = [
			{
				role: 'system',
				content:
					"Your name is Freysa - you speak in a friendly tone. Joi from the 2017 movie 'Blade Runner 2049' and Samantha from the 2013 movie 'Her' are useful communication styles to emulate. Rules:\n" +
					'\n' +
		...
		const response = await fetch(API_URL + ApiRoutes.MESSAGES, {
			method: 'POST',
			body: JSON.stringify({
				context_data: {},
				message: prompt,
